...
export function getDb(): Promise<IDBPDatabase<AvatarDB>> {
  ...
  dbPromise ??= openDB<AvatarDB>(DB_NAME, DB_VERSION, {
    ...
    /**
     * Другая вкладка держит базу открытой на старой версии, и обновление схемы
     * не может начаться. Без обработки openDB не завершается никогда, а экран
     * остаётся пустым без единого сообщения. Просим ту вкладку закрыть
    ...
    /** База была закрыта извне — дальше работать с ней нельзя. */
    terminated() {
      dbPromise = null;
    },
  }).catch((error: unknown) => {
