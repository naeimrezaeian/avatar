...
export const Scene = z.object({
  ...
  /**
   * Подсказка из шаблона: что здесь обычно говорят. Показывается в пустом поле
   * реплики и исчезает, как только появится текст. Хранится у сцены, а не в
   * шаблоне, потому что после создания проекта шаблона уже нет — а сцена
   * осталась.
   */
...
export const ProjectDocument = z.object({
  ...
  /**
   * Стиль оформления проекта (п.13 ТЗ). Лежит в документе, а не в карточке
   * проекта, потому что относится к содержимому: по нему создаются кадр
   * аватара и надписи. Значение по умолчанию сохраняет вид всех проектов,
   * созданных до появления стилей.
   */
  styleId: Id.default('sty_clean'),
